...
static pair<string, string> operator,(path path1, path path2)
{
    if (path1.string() < path2.string())
        return make_pair(path1.string(), path2.string());
...
class AudioSearchEngine
{
  public:
     ...
     *  @return True if a sound file is already in the database
     */
    bool hasSound(path file) { return find(sounds.begin(), sounds.end(), file) != sounds.end(); }
    ...
    void calcAllDistances()
    {
        ...
        for (int i = 0; i < sounds.size(); ++i) {
            for (int j = i; j < sounds.size(); ++j) {
                if (distances[sounds[i], sounds[j]].size() > 0)
                    continue;
                if (j == i)
                    distances[sounds[i], sounds[j]] =
                      vector<double>(AudioFeatures::num_features, 0.);
                else
        ...
        cerr << "Precalculated distances for " << sounds.size() << " sounds in " << duration
             << " seconds" << endl;
        if (use_cache)
            saveDistances();
    }
    ...
    {
        if (distCalculated(a, b))
            return distances[a, b];

        auto dists = Distance::distance(AudioObject(a), AudioObject(b),
                                        thread::hardware_concurrency(), local_align);
        distances[a, b] = dists;
        distances[a, b] = dists;
    ...
    double getWeightedDistance(path a, path b)
    {
        ...
        return 1. -
               net.similarity(FileUtils::getAudioFromAnalysisFile(a),
                              FileUtils::getAudioFromAnalysisFile(b));
    ...
    /**
     *  Get a ranked like of sounds that are nearest to a given query using weighted euclidean
     * distance
    ...
    /**
     *  Mark a sound as "liked" to reweight features. Takes a sound and the query sound and adds
    ...
    {
        // very naive feature reweighting. find feature axis along which the two sounds are closest
        // and weight that
        // feature higher
    ...
    void setUseLocalAlign(bool useLocal) { local_align = useLocal; }

    bool getUseLocal() { return local_align; }
  ...
  private:
    ...
    Distance::CaffeNetSimilarity net;
    /**
    ...
    void printDists()
    {
        //        DBG ("distances.shape: " << distances.shape ()[0] << ", " << distances.shape
        //        ()[1]);
        //
    ...
    void saveDistances()
    {
        if (!new_sounds_added)
            return;
        try {
            path cache_dir(string(getenv("HOME")) + string(APP_SUPPORT) +
                           string(DISTANCE_CACHE_PATH));
            cache_dir = cache_dir.parent_path();
            if (!is_directory(cache_dir))
                create_directory(cache_dir);
            std::ofstream file(string(getenv("HOME")) + string(APP_SUPPORT) +
                               string(DISTANCE_CACHE_PATH));
    ...
    {
        try {
            std::ifstream file(string(getenv("HOME")) + string(APP_SUPPORT) +
                               string(DISTANCE_CACHE_PATH));
