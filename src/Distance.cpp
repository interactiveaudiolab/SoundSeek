...
vector<double> Distance::distance(const AudioObject& a, const AudioObject& b, int num_processors,
                                  bool local_alignment, double dtw_constraint)
{
    ...
    for (int j = 0; j < AudioFeatures::num_features; ++j) {
        ...
        if (local_alignment) {
            //            cout << "calculating distance: smith-waterman" << endl;

            results.emplace_back(thread_pool.enqueue(
              [](vector<double>& s, vector<double>& t, int w, int j) {
                  return pair<int, double>(j, Distance::smithWaterman(s, t));
              },
              a_timeseries, b_timeseries, constraint, j));
        } else {
            //            cout << "calculating distance: DTW" << endl;

            results.emplace_back(thread_pool.enqueue(
              [](vector<double>& s, vector<double>& t, int w, int j) {
                  return pair<int, double>(j, Distance::DTW(s, t, w));
              },
              a_timeseries, b_timeseries, constraint, j));
        }
    }
...
double Distance::weightedPNorm(const vector<double>& dists, const vector<double>& weights, int p)
{
    if (weights.size() != dists.size())
        throw invalid_argument("length of weights and dists must be equal");
...
{
    DBG("calculating distance: smith-waterman");
    ...
    for (int i = 1; i < s.size() + 1; ++i) {
        for (int j = 1; j < t.size() + 1; ++j) {
            double match_mismatch =
              within_tolerance(s[i - 1], t[j - 1], tolerance) ? penalties[0] : penalties[1];
